...
# Magic number based on the fact that 215,000 photos were uploaded in 1 hour on Christmas Day 2023
LARGEST_NUMBER_OF_HOURLY_PHOTOS = 209_999 
...
# flickr ids > ruby's maximum integer so sort based on a string instead of the integer id
sampled_relevant_metadata.sort_by! { |a| a[:thumb_filename] }
output_filename = "ascending-#{File.basename(metadata_filename)}".ext('png')
...
sampled_relevant_metadata.each.with_index do |m, i|
  logger.debug "at index: #{i} uniq count :#{sampled_relevant_metadata.uniq.length}"
